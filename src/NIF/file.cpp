...
namespace NIF
{
	const uint32_t kFormatWidth = 23;

	void File::Header::ExportInfo::ReadBinary(istream& in)
	{
		uint32_t num_info = use_extended_info ? info.max_size() : kNumOldInfo;
	...
	void File::Header::ExportInfo::WriteBinary(ostream& out) const
	{
		uint32_t num_info = use_extended_info ? info.max_size() : kNumOldInfo;
	...
	string File::Header::ExportInfo::ToString() const
	{
		stringstream ss;
		uint32_t num_info = use_extended_info ? info.max_size() : kNumOldInfo;
		...
		for(uint32_t i = 0; i < num_info; ++i)
		{
			ss << setw(kFormatWidth)	<< "Export Info [" + to_string(i + 1)
										+ "]: " << info[i] << endl;
		}
	...
	void File::Header::ReadBinary(istream& in)
	{
		uint8_t test_byte;
		uint32_t vec_size;

		header_line = ReadLine(in);
		ValidateNIF(header_line);
		...
		if(version >= NIFVersion::V10_0_1_0)
		{
			...
			{
				export_info.use_extended_info = true;
			}
			in >> export_info;
		}
	...
	void File::Header::WriteBinary(ostream& out) const
	{
	...
	string File::Header::ToString() const
	{
		stringstream ss;
		uint32_t idx = 0;
		...
		if(user_version_1 >= 10 ||
				((user_version_1 == 1) && version != NIFVersion::V10_2_0_0))
		{
			ss	<< setw(kFormatWidth) << "User Version 2: "
				<< user_version_2 << endl;
		}
		if(version >= NIFVersion::V30_0_0_2)
		{
			ss	<< setw(kFormatWidth) << "Image Preview Size: "
				<< image_preview_size << endl;
		}
		ss << export_info.ToString();
		ss	<< setw(kFormatWidth) << "Number of Block Types: "
			<< block_type_names.size() << endl;
		...
		ss	<< setw(kFormatWidth) << "Number of Block Names: "
			<< block_names.size() << endl;
		ss	<< setw(kFormatWidth) << "Max Block Name Length: "
			<< max_name_length << endl;
		for(const auto& itr : block_names)
		{
			ss	<< setw(kFormatWidth) << "Block Name[" + to_string(++idx) + "]: "
				<< itr << endl;
		}
		if(version >= NIFVersion::V20_1_0_3)
		{
			ss	<< setw(kFormatWidth) << "Number of Groups: "
				<< num_groups << endl;
		}
	...
	string ExtractFileName(const string& file_path)
	{
		string file_name;

		auto folder_end = file_path.find_last_of("/\\");
		auto file_extension = file_path.find_last_of('.');

		if(folder_end != string::npos)
		{
			file_name = file_path.substr(folder_end + 1, file_extension);
		}
		else
		{
			file_name = file_path;
	...
	uint32_t File::GetUserVersion1() const
	{
		return header.user_version_1;
	}

	uint32_t File::GetUserVersion2() const
	{
		return header.user_version_2;
	}
}